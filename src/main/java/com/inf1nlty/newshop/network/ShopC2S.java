...
public final class ShopC2S {
    ...
    {
        Network.sendToServer(new C2SPurchaseRequestPacket(itemID, meta, count));
    }
    ...
    public static void sendGlobalListFromSlot(int itemId, int meta, int amount, int slotIndex, int priceTenths)
    {
        Network.sendToServer(new C2SListFromSlotPacket(itemId, meta, amount, slotIndex, priceTenths));
    }

    public static void sendGlobalListFromContainerSlot(int itemId, int meta, int amount, int containerSlotNumber, int priceTenths, int windowId)
    {
        Network.sendToServer(new C2SListFromSlotPacket(itemId, meta, amount, containerSlotNumber, priceTenths, windowId, true));
    }
    ...
    public static void sendSetPrice(int itemID, int meta, int buyTenths, int sellTenths)
    {
        Network.sendToServer(new C2SSetPricePacket(itemID, meta, buyTenths, sellTenths));
    }
}